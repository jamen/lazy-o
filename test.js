...
test('lazy operations', function (t) {
  ...
  // t.same(lazy('forEach', function (v) { t.is(v, 1, 'triggers ignore symbol method') }, 'skip').run([1]), [1], 'retains value in ignored methods')
  ...
  // t.is(lazy(x => 5, 'skip').run(3), 3, 'allows ignoring plain functions.');
  t.same(lazy('slice', 1, -1)('toUpperCase').run('hello', 'world', 'blah'), ['ELL', 'ORL', 'LA'], 'running multiple values')
  t.end()
})
